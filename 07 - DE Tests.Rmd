...
a<-ggplot(data=as.data.frame(res2))+
  ...
  scale_size_identity()# Easy MA plots.  
...
suppressWarnings(suppressMessages(ggplot(data=reshape2::melt(as.matrix(dfe)))+
  geom_point(aes(x=Var1,y=value,col=Var2))+
  ylim(c(-1,1))+theme_bw(base_size=12)+
  ylab("Deviation from Predicted")))
# Relabel x axis  & legend & make relative deviation instead of absolute 
# Collapse into a boxplot for each mix 
# Bring in the kallisto count table & 
# compare alignment/reference difference between kallisto and star 
