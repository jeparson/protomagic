...
for(fastq in fastqlist){
  if(is.null(p$file(name=fastq)))#check to see if it's already in the cloud folder.  Tested, working.  
    {
    p$upload(filename = paste0(fastqdir,fastq),metadata=as.list(fastqmetadata[fastqmetadata$Fastq_filename==fastq,2:length(fastqmetadata)]))
    }
  }
#upload actually checks that the file was successfully uploaded
